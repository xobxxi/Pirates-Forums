...
function invalidate(params) {
	...
	var css = plugins.dir+'/css/invalidate.css';
	CJAX.script.load(css);
	if(!CJAX.defined(error)) var error = '';
	...
	err =  "<span class='invalidate'><div class='invalidate_message'>"+err+"</div></span>";
	if( !element ) return false;
	...
	{ 
		...
		invalidate_label.style.left = position['left']+element.width+'px';
